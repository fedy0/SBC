...
;					BTYE2		EQU		34H					; 2ND BYTE CODE'S TEMPORARY LOCATION (FOR DATA OR LOWER ORDER BYTE OF AN ADDRESS), FOR 8051 34H = 3RD
;					BYTE3		EQU  	35H					; 3RD BYTE CODE'S TEMPORARY LOCATION (FOR THE HIGH ORDER BYTE OF AN ADDRESS), FOR 8051 35H = 2ND
...
; used for correction
;					submem_keypad_buffer equ   7FH		   OUT OF ABOUT FORTY REMAINING OF 128 RAM MEMORY
...
;					NOTF8085	BIT		07H					; NEXT OPERAND TYPE FLAG (DATA TYPE OPERAND 07H = 0, ADDRESS TYPE OPERAND 07H = 1) FOR 8085 ONLY
...
;					CPUTF		BIT		0DH					; CUP TYPE FLAG (0DH = 0 FOR 8085, 0DH = 1 FOR 8051)
;					NOTF___		BIT		0EH					; NEXT OPERAND TYPE FLAG (DATA TYPE OPERAND 0EH = 0, ADDRESS TYPE OPERAND 0EH = 1) FOR 8051 ONLY
;					VNF___		BIT		0FH					; VALID NUMBER FLAG. IF BIT ADDR 0FH = 1 THE KEY PRESSED IS A VALID NUMBER ELSE INVALID. FOR 8051 ONLY
;					OTF1___		BIT		10H					; OPERAND TYPE FLAG 1	 FOR 8051
...
MAIN:														; THE MAIN PROGRAM BEGINS HERE
					MOV			SP, #57H					; INITIALIZATION OF THE TOP OF THE STACK(40 BYTES OF MEM. ARE AVAILABLE ON THIS INITIALIZATION)
					MOV 		P1,#0FFH					; MAKE PORT1 AN INPUT PORT
					MOV 		P3, #11001100B				 ; corrected. formerly was 	#00001000B
					LCALL 		LCD_SET						; SET THE LCD FOR PROPER PERFORMANCE
					LCALL		INTRO_MSG					; CALLS AN OSR TO DISPLAY LINE 1: '   SBC 8085uP   '
															;                         LINE 2: 'by IFEDIORA E.C.'  OR 'by FEDYRONIX INC'
MAIN1:				CALL		KEYPAD_ID					; ENSURE A KEY IS DEPRESSED TO CONTINUE
...
MAIN1_5:			SETB		IT0							; corrected. this line wasn't here before
					CLR			IE0							; corrected. this line wasn't here before
					MOV 		IE, #10000001B				; ENABLE THE !INT0 EXTERNAL INTERRUPT (SECOND_FUNCTION ISR)

					CLR_LCD
					LCALL		INTRO_SUBMEM_REQUEST		; CALLS AN OSR TO DISPLAY ON LINE 1: 'Enter start addr'
; corrected to 50 times with 20mS delay. Not also that the label for the delay was changed from LCD_DLY gto DELAY_20mS
					MOV			30H,#32H					; LOAD '50DEC IN HEX (32H)' INTO THE COUNTER (50 TIMES) 
					MOV 		P3, #11001100B				 ; corrected. formerly was 	#00001000B

					JNB			00H, $						; STAY HERE TILL THE SUB MEM IS PRESSED
; corrected
					CLR			00H							; refreshes sub mem flag. this line wasn't here before
					; 8085 OR 8051?
					JB			0DH, ___8051				; IF 0DH = 1 (8051) JUMP TO 8051 ASM/INSTR. SET ELSE CONTINUE WITH 8085 ASM/INSTR. SET
...
; =*=*=*=*=*=*=*=*=*=*=*=*=*        ASSEMBLY LANGUAGE AS THE CODE/PROGRAMMING LANGUAGE TYPE (CPLT) FOR 8085        =*=*=*=*=*=*=*=*=*=*=*=*=*=

MAIN2:				MOV			A, #0C0H					; INSTRUCTION/COMMAND TO CARRY OUT DISPLAY ON LINE 2
					CALL		LCD_CMD
					; INCLUDE CODE TO REFRESH SOME MEM LOCATION USED AS COUNTER IN THE PROGRAM
...
SECOND_FUNCTIONcorrected:
					CLR			01H							; GET READY FOR THE NEXT KEY
;corrected;
;					mov			A, 7FH						; RETURN THE SAVED VALUE
	        		mov			A, 31H						; RETURN THE TEMPORARY SAVE ASCII CHARACTERR OF THE KEY IDENTIFIED
;corrected;
...
EXIT_SECOND_FUNCTION:
					MOV			30H,#32H					; REFRESH OR RE-INITIALIZE COUNTER BY LOADING '50dec (ie. 32H)' INTO THE COUNTER 
				   	RETI

; =================================ORDINARY SERVICE ROUTINE (OSR)====================================================================
ADDRESS_DISPLAY_CONTROL:
; THIS SR IS USED BY FLY_OPCODE & FLY_OPERAND (ie DATA TYPE & ADDRESS TYPE) TO CONTROL THE ADDRESS DISPLAY IN LINE 1
; WHENEVER ENTRY IN LINE 2 IS COMPLETE AND NEXT IS PRESSED.
; ALSO IT IS USED BY V_HEX AND CANCEL SR TO CONTROL THE ADDRESS DISPLAY IN LINE 1
...
NEXT:			
; corrected. this 2 lines of code just below were formerly not here
				PUSH		DPH							; SAVE THE CONTENT OF THE DATA POINTER REGISTERS SINCE THEY WOULD BE USED HERE
				PUSH		DPL
...
NEXT_4_SUB_MEM:
				...
				MOV			39H, 34H					; LOAD THE LOW ORDER BYTE INTO NEXT ADDRESS HOLDER/ADDRESS_DISPLAY_CONTROL'S INPUT MEM LOC 1 FOR DISPLAY
; corrected. this 2 lines of code just below were formerly not here
; see if the 2 lines of code just above can be ignored for the sake of optimizing the 'next sub mem sr' section
;	   			MOV			DPH, 35H					; LOAD THE HIGH ORDER BYTE
;				MOV			DPL, 34H					; LOAD THE LOWER ORDER BYTE
; ignore by using the commented code just above instead of the two just below
				MOV			DPH, 3AH					; LOAD THE HIGH ORDER BYTE
				MOV			DPL, 39H					; LOAD THE LOWER ORDER BYTE
				...
				SJMP		EXIT_NEXT					; ELOO CHECK AGAIN COS I THINK IT IS NECESSARY TO BE CLEARED ON MY FIRST LOOK/REVIEW 
NEXT_4_OPCODE:
; corrected. this 2 lines of code just below were carried to the top of the NEXT OSR entrance to accommodate the sub mem ISR
;				PUSH		DPH							; SAVE THE CONTENT OF THE DATA POINTER REGISTERS SINCE THEY WOULD BE USED HERE
;				PUSH		DPL
...
;corrected
					JB			00H, FA_EXIT_1 ;formerly FA_EXIT	; SKIP THE SR SEGMENT BELOW IF IT IS A SUB MEM CALL
...
;corrected. there was no label just below, before
FA_EXIT_1:			CALL		NEXT						; CALL NEXT OSR TO PERMANENTLY SAVE THE OPERAND CODES
					JB			00H, FA_EXIT  ; this line never existed before
...
; ELOO U MAY PUT UR FLAG FOR SUB MEM ASCII_2_HEX SR CALL HERE(NOTE SUCH FLAG MUST BE CLEAR AT THE SUB MEM SR ON RETURN IMMEDIATELY)
; EXPECTED FLAG INSTRUCTION: JB		SubMem'sAscii2hexFlag, EXIT_A2H
; corrected the above instruction is added below that for the generous comment	08H can still be used
;				JB		00H, EXIT_A2H
; THE TEST IS COMPLETED. THE INEQUQLITY: 30H =< A =< 46H HAS BEEN SATISFIED AND CONVERSION HAS TAKEN PLACE!!!
...
E_N_D:
					...
					DB			' Please Wait... ',0
; corrected
					LCALL		DELAY_2S					; just added this line to ensure that ' Please Wait... ' displayed
; corrected
...
; ----------------------------------------------------------------------------------------------------------------------------------
; 2S DELAY FOR END OSR	(CALL THE PROGRAM ABOVE 2 TIMES). OPTIMIZATION DONE.
DELAY_2S:		MOV		3DH, #64H				; MOVE #64H (100DEC) INTO MEM LOC. 3DH FOR DELAY (100 x 20mS = 2S)
				; 20mS DELAY
DLY_2S_1:		MOV		3BH, #3DH				; MOVE #0FFH INTO MEM LOCATIONS (3BH & 3CH) THAT SERVE 
DLY_2S_2:		MOV		3CH, #0A2H				; AS COUNTER FOR THIS PARTICULAR DELAY SUBROUTINE
				DJNZ	3CH, $					; STAY HERE TILL THE CONTENT OF THE MEMORY LOCATION (3CH) IS ZERO
				DJNZ	3BH, DLY_2S_2			; STAY HERE TILL THE CONTENT OF THE MEMORY LOCATION (3BH) IS ZERO
				; --- END
				DJNZ	3DH, DLY_2S_1			; STAY HERE TILL THE CONTENT OF THE MEMORY LOCATION (3BH) IS ZERO
...
;
					D_STR									; A LCD SCREEN CLEAR THUS DDRAM ADDRESS AUTOMATICALLY RESETS TO LINE 1
					DB			'Press 3 for 8085',0		; LINE 1
					MOV			A, #0C0H					; SET DDRAM ADDRESS TO LINE 2
					CALL		LCD_CMD				  
					D_STR									; DISPLAY THE FOLLOWING STRINGS
					DB			'Press 4 for 8051',0		; LINE 2
					RET
; ----------------------------------------------------------------------------------------------------------------------------------
; **********************************************
	       			ORG 		30H
